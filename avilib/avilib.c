...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   OUTLONG(1000*AVI->mp3rate/8);
   OUTLONG(0);             /* Start */
   OUTLONG(4*AVI->audio_bytes/sampsize);   /* Length */
   OUTLONG(0);             /* SuggestedBufferSize */
   OUTLONG(-1);            /* Quality */
   ...
   while(1)
   {
      ...
      if(strncasecmp(data,"LIST",4) == 0)
      {
         ...
         if(strncasecmp(data,"hdrl",4) == 0)
         {
            ...
            // offset of header
            AVI->a_codecf_off = lseek(AVI->fdes,0,SEEK_CUR); 
   ...
   for(i=0;i<hdrl_len;)
   {
      ...
      else if(strncasecmp(hdrl_data+i,"strf",4)==0)
      {
         ...
         else if(lasttag == 2)
         {
            ...
            AVI->mp3rate = 8*str2ulong(hdrl_data+i+8);
            //:ThOe
...
{
   return AVI->a_codech_off;
}

long AVI_audio_codecf_offset(avi_t *AVI)
{
   return AVI->a_codecf_off;
}
...
long AVI_audio_size(avi_t *AVI, long frame)
{
  ...
  if(frame < 0 || frame >= AVI->audio_chunks) return 0;
...
int AVI_is_keyframe(avi_t *AVI, long frame)
{
   if(AVI->mode==AVI_MODE_WRITE) { AVI_errno = AVI_ERR_NOT_PERM; return -1; }
   if(!AVI->video_index)         { AVI_errno = AVI_ERR_NO_IDX;   return -1; }

   if(frame < 0 || frame >= AVI->video_frames) return -1;
   return (AVI->video_index[frame].key==0x10) ? 1:0;
}
